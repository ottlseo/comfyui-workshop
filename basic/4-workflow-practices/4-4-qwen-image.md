...
- Qwen Image Edit으로 이미지의 특정 부분을 자연스럽게 편집하기
- Qwen Image Layered로 이미지를 여러 레이어로 분해하기
- 마스크를 사용하여 편집 영역을 지정하는 방법 익히기
- 두 모델을 조합하여 고급 편집 워크플로우 구성하기
...
**Qwen Image Edit 파트:**
- [ ] 필요한 모델 파일을 다운로드하고 올바른 경로에 배치했다
- [ ] 이미지 편집 워크플로우를 로드하고 실행했다
- [ ] 편집 프롬프트를 작성하여 이미지 특정 부분을 변경했다
- [ ] Denoise, CFG Scale 파라미터를 조정하며 결과 차이를 확인했다

**Qwen Image Layered 파트:**
- [ ] 이미지를 여러 레이어로 성공적으로 분해했다
- [ ] 분해된 레이어를 확인하고 투명도(알파 채널)를 이해했다
- [ ] (선택) 두 모델을 조합한 고급 워크플로우를 시도했다

> **초보자 팁**: Qwen Image Edit은 "change the shirt to red" 같은 간단한 프롬프트로 시작하세요. 너무 복잡한 편집을 한 번에 요청하지 말고, 한 가지씩 차근차근 변경하는 것이 좋습니다.
