...
- ControlNet의 개념과 작동 원리 이해하기
- 다양한 ControlNet 모델(OpenPose, Canny, Depth 등)을 활용하기
- 전처리기를 사용하여 일반 이미지를 ControlNet 입력으로 변환하기
- ControlNet 강도를 조정하여 원하는 결과 얻기
...
> **초보자를 위한 비유**: ControlNet은 **밑그림 위에 색칠하기**와 같습니다. 밑그림(참조 이미지)의 윤곽선이나 포즈를 따라가면서, 그 위에 AI가 새로운 색과 디테일을 입히는 것이죠. 사진 속 사람의 포즈를 그대로 유지하면서 다른 스타일의 캐릭터로 바꾸거나, 건물의 윤곽선을 유지하면서 다른 디자인을 입힐 수 있습니다.
...
> **초보자 팁**: 처음에는 **0.7**로 시작하세요. 이 값이 가장 안정적입니다. 결과를 보고 "참조 이미지를 더 정확히 따라야 한다"고 느끼면 1.0으로 올리고, "너무 딱딱하게 따른다"고 느끼면 0.5로 낮추세요.
...
- [ ] ControlNet 모델을 다운로드하고 Load ControlNet Model 노드에서 로드했다
- [ ] 전처리된 이미지로 ControlNet을 성공적으로 적용해봤다
- [ ] 전처리기를 사용하여 일반 이미지를 OpenPose로 변환해봤다
- [ ] strength 값을 0.5, 0.7, 1.0 등으로 바꿔보며 차이를 확인했다
- [ ] 최소 2가지 이상의 ControlNet 타입(예: OpenPose, Canny)을 실습해봤다
