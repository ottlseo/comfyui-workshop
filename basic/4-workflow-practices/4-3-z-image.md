...
- Z-Image Turbo 모델의 특징과 장점 이해하기
- 필요한 모델 파일을 다운로드하고 올바른 경로에 배치하기
- 워크플로우 템플릿을 로드하고 이미지 생성하기
- FP8 양자화 모델을 사용하여 저사양 환경에서 실행하기
...
- [ ] 3개의 필요한 모델 파일을 모두 다운로드하고 올바른 경로에 배치했다
- [ ] 워크플로우 템플릿을 성공적으로 로드했다
- [ ] 프롬프트를 입력하고 이미지를 생성했다
- [ ] Steps, CFG Scale 등 파라미터를 변경해보며 결과 차이를 확인했다
- [ ] (선택) FP8 양자화 모델을 사용하여 속도 향상을 경험했다

> **초보자 팁**: Z-Image Turbo는 다른 모델에 비해 설정이 간단하고 결과가 빠르게 나옵니다. ComfyUI 워크플로우에 익숙해지는 연습용으로 아주 좋습니다!
